...
class IDENTITY(object):
	...
	def storage_driver(self, ini_file, config_file):
		config1 = RawConfigParser()
		config1.read(ini_file)
		file = fileinput.FileInput(config_file, inplace=1)
		for section in config1.sections():
			for key in config1.options(section):
				value = config1.get(section, key)
				written = 0
				section_found = 0
				for line in file:
					if written == 0 and section_found == 0 and line.startswith(section):
						section_found = 1
					if written == 0 and section_found == 1 and key in line and line.startswith(key):
						line = line.replace(line,"\t"+key+" = "+value+"\n")
						written = 1					
				 	if written == 0 and section_found == 1 and line.startswith('}'):
						line = line.replace(line, "\t"+key+" = "+value+"\n}\n\n")
						written = 1
					sys.stdout.write(line)
        	file.close()
